        ...
        target: creep =>{
            if(creep.room.name == 'W39N9'){
                ...
                if(enemy){
                    ...
                    var ram = creep.room.lookForAt(LOOK_STRUCTURES,14,19,{
                        filter: s => s.structureType == STRUCTURE_RAMPART
                    })
                    if(ram.length>0){
                        if(creep.attack(ram[0])==ERR_NOT_IN_RANGE){
                            creep.moveTo(ram[0])
                        } 
                    }
                    else{
                        var site = creep.pos.findClosestByPath(FIND_HOSTILE_CONSTRUCTION_SITES)
                        if(site){
                            creep.moveTo(site)
                        }
                        else{
                            var s=Game.getObjectById('5ef1f8b443252b8128dd46fb')
                            if(s){
                                if(creep.attack(s)==ERR_NOT_IN_RANGE){
                                    creep.moveTo(s)
                                }
                            }
                            else{
                                var s=creep.pos.findClosestByPath(FIND_HOSTILE_STRUCTURES)
                                if(s){
                                    if(creep.attack(s)==ERR_NOT_IN_RANGE){
                                        creep.moveTo(s)
                                    }
